...
from .models import AgentState, Panel
...
def story_understanding(state: AgentState):
    ...
    for batch_start in range(0, len(sorted_page_nums), BATCH_SIZE):
        ...
        Tu tarea es leer el siguiente fragmento de guión y producir DOS tipos de análisis:
        
        ### 1. RESUMEN DETALLADO POR PÁGINA DE GUIÓN
        Para cada página del guion de cómic identificada en el fragmento, genera un resumen visual detallado que describa:
...
def planner(state: AgentState):
    ...
    if existing_chars or existing_scenes:
        ...
        assets_context += "Si el guión requiere un escenario o personaje que no está en esta lista, intenta usar el más parecido, pero si se diferencia mucho, no inventes nuevos escenarios ni personajes."
...
def image_generator(state: AgentState):
    ...
    # Preserve the current continuity semantics by resolving it sequentially first.
    ...
    def resolve_panel_job(job, adapter_override=None):
        ...
        with timed_step(f"image_generator.prompt_build[{panel_id}]"):
            augmented_prompt = pb.build_panel_prompt(panel, state["world_model_summary"], panel_continuity)
